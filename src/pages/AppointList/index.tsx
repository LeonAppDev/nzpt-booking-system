...
const renderList = (list: OrderDetailList, index: number, classes:any) => (
...
class AppointList extends Component<AppointListProps> {
    ...
    {   const { highTea, classes } = this.props;
       ...
       /* if(listsAscByDate)
            {
            return(<div>
                    { listsAscByDate.map((list, index) => renderList(list, index))}
                    </div>)}
                        else
                        {
...
*/  
    return (<div>
        {listsAscByDate?listsAscByDate.map((list, index) => renderList(list, index,classes)):<CircularProgress className={classes.progress} color="secondary" />}
        </div>)
} 
}
